...
<html>
  ...
  String sql="select *from preorder where serId = '"+id+"'";
 ResultSet rs=stmt.executeQuery(sql);
   while(rs.next()){
 String havSignedAgency=rs.getString("reaNumber");
 String havVisited=rs.getString("preNumber"); 
 double a=Double.parseDouble(havSignedAgency);//设置数据集
 double b=Double.parseDouble(havVisited);//设置数据集
 double c=a/b;
     DefaultPieDataset dataset = new DefaultPieDataset();
     dataset.setValue("实际采购比率",c);
     dataset.setValue("剩余比率", 1-c);
     //通过工厂类生成JFreeChart对象
     JFreeChart chart = ChartFactory.createPieChart3D("经销商实际采购率分布图",
