...
<table align="center" border="0" cellspacing="1">
	...
	<tr>
		...
		<td align="center"><label><a href='confirmDeletepill.jsp?id=<%=row.getString("medId")%>' target="mainFrame">删除</a></label></td>
	</tr>				
	<% 		
				if(!row.next())break;
			}
...
请输入要管理的药品编号： <input name="DI" type="text" maxlength="10">
<input name="sure" type ="submit"  value="提交">
    </form>			
    <div align="center"><a href='register2.jsp?'  target="mainFrame">添加药品 |</a><a href='manageproduct.do?id=<%=str1 %>'  target="mainFrame">上一页 |</a><a href='manageproduct.do?id=<%=str2 %>'  target="mainFrame"> 下一页</a>&nbsp;&nbsp; 药品列表共<%=pageCount %>页&nbsp; <%=rowCount %>个药品&nbsp; 当前是第<%=currentPage %>页
  <%  	}
